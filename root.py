        ...
        for p in self.get_pillars():
            ...
            for c in p.get_cell_pillar():
                ...
                for cell_fragment in cell_fragmentation:
                    ...
                    v = 0.9
                    pp = p.get_type()
                    #print(type_lib.type_of_column(pp))
            ...
            for c in p.get_cell_pillar():
                ...
                for cell_fragment in cell_fragmentation:
                    ...
                    def dydt(c_loc, t, y0, c_neigh, h, H, D, pillar_square,
                        ...
                        volume = self.get_cell(i, j).get_length() * pillar_square
                        for i in range(len(H)):
                            function += (c_neigh[i] - c_loc) * h / H[i] * D[i]
                        function = function / volume
                        function += bot * (c_bot - c_loc) * pillar_square * D_loc / volume
                        function += top1 * (c_top - c_loc) * pillar_square * D_loc / volume
                        function = function - v * c_loc * 0 + a * c_loc * 0
                        return function
                    ...
                    v = 0.9
                    pp = p.get_type()
                    local_cell_type = type_lib.type_of_column(pp)
                    pillar_square = type_lib.area(pp)
                    ...
                    for pil, cel in fragment_neighbours.items():
                        ...
                        local_cell = self.get_cell(pil - 1, cel)
                        H_i = local_cell.get_length()
                        neighbour_cell_type = type_lib.type_of_column(pil)
                        #print('aaa', neighbour_cell_type)
                        H.append(H_i)
                        if local_cell_type == neighbour_cell_type:
                            D.append(0.5)
                        else:
                            D.append(0.25)
